...
public partial class KismetScriptCompiler
{
    ...
    {
        // Check if this identifier is a symbolic label/offset name (e.g., "FunctionName_Offset" or "FunctionName_Offset_CallingFunction")
        // These are generated by the decompiler's FormatCodeOffset method and represent code offsets as integers
        ...
        if (parts.Length >= 2)
        {
            ...
            for (int i = parts.Length - 1; i >= 0; i--)
            {
                ...
                {
                    // This looks like a symbolic offset name - try to resolve as a label first
                    var labelSymbol = GetSymbol<LabelSymbol>(identifier.Text);
                    if (labelSymbol != null)
                    {
                        // Found a defined label - use it
                    ...
                    {
                        // No defined label - treat as a numeric constant representing the offset
                        // This handles cases where the decompiler generated symbolic names for offsets
                        // that don't correspond to actual label definitions in the decompiled code
                        return Emit(identifier, new EX_IntConst()
                        {
                            Value = offset
                        });
