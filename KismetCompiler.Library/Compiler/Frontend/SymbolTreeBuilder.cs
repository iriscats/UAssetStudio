...
namespace KismetCompiler.Library.Compiler.Frontend;
...
public class SymbolTreeBuilder
{
        ...
        foreach (var decl in compilationUnit.Declarations)
        {
            ...
            if (importAttrib != null)
            {
                if (importAttrib.Arguments.Count != 1)
                    throw new KismetCompiler.Library.Compiler.Exceptions.UnexpectedSyntaxError(importAttrib);
                var packagePath = importAttrib.Arguments[0].Expression as StringLiteral;
                if (packagePath == null)
                    throw new KismetCompiler.Library.Compiler.Exceptions.UnexpectedSyntaxError(importAttrib.Arguments[0]);
    ...
    private static KismetCompiler.Library.Compiler.FunctionCustomFlags GetCustomFunctionFlags(ProcedureDeclaration procedureDeclaration)
    {
        var functionFlags = (KismetCompiler.Library.Compiler.FunctionCustomFlags)0;
        foreach (var attr in procedureDeclaration.Attributes)
        {
            if (!System.Enum.TryParse<KismetCompiler.Library.Compiler.FunctionCustomFlags>(attr.Identifier.Text, out var flag))
                continue;
            functionFlags |= flag;
        }
        return functionFlags;
